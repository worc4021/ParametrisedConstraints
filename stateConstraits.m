...
% paraMax = 20;
% 
% positivity = [zeros(sizes.nX),-eye(sizes.nAlpha+sizes.nBeta);
%               zeros(sizes.nX),eye(sizes.nAlpha+sizes.nBeta)/paraMax];
% 
% Vpost = [];
% vpost = [];
% 
% Vpre = [V;positivity];
% vpre = [v;ones(2*sizes.nX,1)];
% 
...
%     end
% 
% [Vpost,vpost] = linReduce([Vpre;Vn;positivity],[vpre;vn;ones(2*sizes.nX,1)]);
...
for  i = 2:nHor
    ...
    [tLam,tlam] = PreConditioning(tLam,tlam);
    [Lam{i},lam{i}] = linReduce(tLam,tlam);
end
